...
bool loadPoses(string file_name, vector<Matrix> &poses) {
...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  for (int32_t first_frame=0; first_frame<poses_gt.size(); first_frame+=step_size) {
    ...
    for (int32_t i=0; i<num_lengths; i++) {
      ...
      std::cout << "first_frame=" << first_frame << " last_frame=" << last_frame << " len=" << len;
      ...
      if (last_frame==-1)
	{
      ...
      std::cout << " r_err: " << r_err << " t_err:" << t_err << std::endl;
      std::cout << "pose_delta_gt: " << pose_delta_gt << endl;
      std::cout << "pose_delta_result: " << pose_delta_result << endl;
      std::cout << "pose_delta_error: " << pose_error << endl;
      // write to file
...
void savePathPlot (vector<Matrix> &poses_gt,vector<Matrix> &poses_result,string file_name) {
  ...
  for (int32_t i=0; i<poses_gt.size(); i+=step_size)
    fprintf(fp,"%d %f %f %f %f\n",i, poses_gt[i].val[0][3],poses_gt[i].val[2][3],
...
void plotPathPlot (string dir,vector<int32_t> &roi,int32_t idx) {
  ...
  for (int32_t i=0; i<2; i++) {
    ...
    fprintf(fp,"plot \"%02d.txt\" using 2:3 lc rgb \"#FF0000\" title 'Ground Truth' w lines,",idx);
    fprintf(fp,"\"%02d.txt\" using 4:5 lc rgb \"#0000FF\" title 'Visual Odometry' w lines,",idx);
    fprintf(fp,"\ '' using 2:3:1 with labels offset -2,0,");
    fprintf(fp,"\"< head -1 %02d.txt\" using 2:3 lc rgb \"#000000\" pt 4 ps 1 lw 2 title 'Sequence Start' w points\n",idx);
...
bool eval (string prefix, string result_sha,Mail* mail) {

  char *gt_dir_        = getenv("ODO_GT_DIR");
  char *result_dir_    = getenv("ODO_RES_DIR");

  // ground truth and result directories
  string gt_dir         = (!gt_dir_) ? "/home/ryu/catkin_limo/src/limo" : gt_dir_;
  string result_dir     = (!result_dir_) ? "/home/ryu/catkin_limo/src/limo/result" : result_dir_;
  ...
  for (int32_t i=4; i<5; i++) {
    ...
    // file name
    char file_name[256];
    sprintf(file_name,"%s%02d.txt", prefix.c_str(), i);

    // read ground truth and result poses
    vector<Matrix> poses_gt, poses_result;

    if (!loadPoses(gt_dir + "/" + file_name, poses_gt) ||
	!loadPoses(result_dir + "/" + "test.txt", poses_result))
      {
	continue;
      }
    // plot status
    ...
    // check for errors
    if (poses_gt.empty() || poses_result.empty()) {
    ...
    if (poses_result.size()!=poses_gt.size()) {
      poses_gt.resize(poses_result.size());
    }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // read arguments
  string result_sha = argv[1], prefix = "";
  ...
  // run evaluation
  bool success = eval(prefix, result_sha,mail);
