...
void MonoLidar::callbackSubscriber(const TrackletsMsg::ConstPtr& tracklets_msg,
    ...
    std::string filePath = "/home/ryu/catkin_limo/src/limo/test.txt";
    ...
    if (bundle_adjuster_->keyframes_.size() > 0) {
        ...
        if (has_external_prior) {
            ...
            ss << "Duration 5 point="
              ...
              //std::cout << "eval_ : \n" << motion_vehicle_t1_t0.translation(0,0)<< " " << motion_vehicle_t1_t0.translation(0,1) << std::endl;
               //std::cout << "eval_trans : \n" << motion_vehicle_t1_t0.translation() << std::endl;
               //std::cout << "eval_linear : \n" << motion_vehicle_t1_t0.linear() << std::endl;
               //writeFile.close();
