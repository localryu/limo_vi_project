...
bool eval (string result_sha,Mail* mail) {
  ...
  string gt_dir         = "/home/ryu/catkin_limo/src/limo/pose_gt";
  string result_dir     = "/home/ryu/catkin_limo/src/limo/result" + result_sha;
  ...
  for (int32_t i=4; i<5; i++) {
    ...
    vector<Matrix> poses_result = loadPoses(result_dir + "/" + "test.txt");
    ...
    if (poses_gt.size()==0 || poses_result.empty()) {
